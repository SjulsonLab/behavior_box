...
%% parameters to set
startTrialNum          = 2;     % in case you stop and start on the same day
resetTimeYN            = 'yes'; %

m.mouseName            = 'debugging';
m.trainingPhase        = 4;
m.low                  = 'right';  % this is specific for each animal and must be consistent between sessions
m.serialPort           = 'COM6'; % look this up in the arduino software
...
if strcmpi(computer, 'MACI64')
   ...
   m.basedir = 'D:\Google Drive\lab-shared\lab_projects\rewardPrediction\behavior';
end
...
while toc(t)/60 < m.sessionLength && nTrial <= m.maxTrials && exitNowYN == 0 && exitAfterTrialYN == 0
   ...
   if m.trainingPhase <= 5 || m.trainingPhase == 7
      if LcueProb > round(rand()*99) % left cue
