...
if session_info.trainingPhase > 1
	f1 = PokePlot(session_info,trialAvailable,Lpokes,Rpokes,Ipokes,basename,Lrewards,Rrewards,1,trialStart);
else
...
close all
histbin = 2; % in ms
histvec = (0:histbin:trialAvailable(end)+120);
hist_scale = 60000 ./ histbin;
...
% histvec = 0:100:trialAvailable(end)+120; %EFO: testing/ it looks weird
norm = 'count';
...
%    histvec1 = histvec;
% end
h1 = plot(histvec/hist_scale, Lhist, 'b'); hold on
...
h2 = plot(histvec/hist_scale, Rhist, 'r');
h2.Color(4) = 0.5;
...
% end
h3 = plot(histvec/hist_scale, Ihist, 'g');
h3.Color(4) = 0.5;
...
% a(2) = subplot(3,1,2);
% 
% % h4 = bar(histvec/hist_scale, Thist*2, 'k'); hold on % used to plot trial boundaries this way
% % plot rewards
...
% % end
% h5 = plot(histvec/hist_scale, LrewardHist, 'b'); hold on
% h5.Color(4) = 0.5;
% % h5 = area(histvec/hist_scale, LrewardHist);
% a(2).YLim = [-0.2 1.2];
% 
...
% % end
% h6 = plot(histvec/hist_scale, RrewardHist, 'r');
% h6.Color(4) = 0.5;
% 
% 
% a(2).XTickLabel = [];
...
L3 = legend('Trial Available', 'L rewards', 'R rewards', 'all rewards','Trial Started');
